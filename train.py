...
if args.vimeo90k:
    trainset = dataloader.SeDraw_vimeo90k(root=args.dataset_root, transform=transform, train=True)
else:
    trainset = dataloader.SeDraw(root=args.dataset_root + '/train', transform=transform, train=True)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.train_batch_size * device_count, shuffle=True)

if args.vimeo90k:
    validationset = dataloader.SeDraw_vimeo90k(root=args.dataset_root, transform=transform,
                                      randomCropSize=(448, 256), train=False)
else:
    validationset = dataloader.SeDraw(root=args.dataset_root + '/validation', transform=transform,
                                      randomCropSize=(448, 256), train=False)
...
for epoch in range(dict1['epoch'] + 1, args.epochs):
    ...
    if (epoch % args.checkpoint_epoch) == args.checkpoint_epoch - 1:
        ...
        torch.save(dict1, args.checkpoint_dir + "/SeDraw" + str(checkpoint_counter) + ".ckpt")
