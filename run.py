...
def ToImage(frame0, frame1):
    ...
    with torch.no_grad():
        ...
        ref_imgt, edge_ref_imgt = structure_gen((img0_e, img1_e))
        imgt = detail_enhance((img0, img1, ref_imgt))
        # imgt = detail_enhance((img0, img1, imgt))
        imgt = torch.clamp(imgt, max=1., min=-1.)

    return imgt, ref_imgt, edge_ref_imgt, img0_e[:, 3:, :, :].repeat(1, 3, 1, 1), img1_e[:, 3:, :, :].repeat(1, 3, 1, 1)
...
def main():
    ...
    for folder in tqdm(os.listdir(args.imgpath)):
        ...
        first, second = X0, X1
        imgt, ref_imgt, edge_ref_imgt, edge0, edge1 = ToImage(first, second)
        ...
        ref_imgt_np = ref_imgt.squeeze(
            0).cpu().numpy()  # [:, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth]
        ref_imgt_png = np.uint8(((ref_imgt_np + 1.0) / 2.0).transpose(1, 2, 0)[:, :, ::-1] * 255)
        edge_ref_imgt_np = edge_ref_imgt.squeeze(
            0).cpu().numpy()  # [:, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth]
        edge_ref_imgt_png = np.uint8(((edge_ref_imgt_np + 1.0) / 2.0).transpose(1, 2, 0)[:, :, ::-1] * 255)
        edge0_np = edge0.squeeze(
            0).cpu().numpy()  # [:, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth]
        edge0_png = np.uint8(((edge0_np + 1.0) / 2.0).transpose(1, 2, 0)[:, :, ::-1] * 255)
        edge1_np = edge1.squeeze(
            0).cpu().numpy()  # [:, intPaddingTop:intPaddingTop+intHeight, intPaddingLeft: intPaddingLeft+intWidth]
        edge1_png = np.uint8(((edge1_np + 1.0) / 2.0).transpose(1, 2, 0)[:, :, ::-1] * 255)
        if not os.path.isdir(triple_path):
            os.system('mkdir -p %s' % triple_path)
        cv2.imwrite(triple_path + '/SeDraw.png', imgt_png)
        cv2.imwrite(triple_path + '/SeDraw_ref.png', ref_imgt_png)
        cv2.imwrite(triple_path + '/SeDraw_edge_ref.png', edge_ref_imgt_png)
        cv2.imwrite(triple_path + '/SeDraw_edge0.png', edge0_png)
        cv2.imwrite(triple_path + '/SeDraw_edge1.png', edge1_png)
